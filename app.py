...
def plotChart(df, lst2, num1, num2, x_fake, df_dx, optionOrderList, stockName='', prevdtstr:str='', sord:list=[], trends:list=[], lstVwap:list=[], bigOrders:list=[], pea:bool=False, timeStamp:int=None, previousDay:bool=False, OptionTimeFrame:list=[], overall:list=[]):
    ...
    #if 2 in lstVwap:
    fig.add_trace(go.Scatter(x=df['time'], y=df['STDEV_2'], mode='lines', opacity=0.3, name='UPPERVWAP2'))
    fig.add_trace(go.Scatter(x=df['time'], y=df['STDEV_N2'], mode='lines', opacity=0.3, name='LOWERVWAP2'))
    #if 0 in lstVwap:
    fig.add_trace(go.Scatter(x=df['time'], y=df['STDEV_25'], mode='lines', opacity=0.3, name='UPPERVWAP2.5'))
    fig.add_trace(go.Scatter(x=df['time'], y=df['STDEV_N25'], mode='lines', opacity=0.3, name='LOWERVWAP2.5'))
    #if 1 in lstVwap:    
    fig.add_trace(go.Scatter(x=df['time'], y=df['STDEV_1'], mode='lines', opacity=0.3, name='UPPERVWAP1'))
    fig.add_trace(go.Scatter(x=df['time'], y=df['STDEV_N1'], mode='lines', opacity=0.3, name='LOWERVWAP1'))
        
    #if 1.5 in lstVwap:     
    fig.add_trace(go.Scatter(x=df['time'], y=df['STDEV_15'], mode='lines', opacity=0.3, name='UPPERVWAP1.5'))
    fig.add_trace(go.Scatter(x=df['time'], y=df['STDEV_N15'], mode='lines', opacity=0.3, name='LOWERVWAP1.5'))

    fig.add_trace(go.Scatter(x=df['time'], y=df['STDEV_0'], mode='lines', opacity=0.3, name='UPPERVWAP0.5'))
    fig.add_trace(go.Scatter(x=df['time'], y=df['STDEV_N0'], mode='lines', opacity=0.3, name='LOWERVWAP0.5'))

    fig.add_trace(go.Scatter(x=df['time'], y=df['1ema'], mode='lines', opacity=0.3, name='1ema',marker_color='rgba(0,0,0)'))
    ...
    if len(optionOrderList) > 0:
        ...
        fig.add_trace(
            go.Bar(
                ...
                hovertext=pd.Series([i[0] for i in newOpp]),
            ),
            row=1, col=3
        )
...
def update_graph_live(n_intervals):
    ...
    '''
    try:
        with open('GetAllTrades'+stkName+'.csv', 'r', encoding="utf8") as f:
              reader = csv.reader(f)
              AllTrade = list(reader)
    ...
    try:
        '''
        with open('dailyCandle'+stkName+'.csv', 'r', encoding="utf8") as f:
              reader = csv.reader(f)
              fft = list(reader)
        '''      
        gclient = storage.Client(project="stockapp-401615")
        ...
        '''
        with open('OptionTimeFrame'+stkName+'.csv', 'r', encoding="utf8") as f:
              reader = csv.reader(f)
              OptionTimeFrame = list(reader)
        '''      
        gclient = storage.Client(project="stockapp-401615")
        bucket = gclient.get_bucket("stockapp-storage")
